...
void calc_derived_iat(flow_feature_state_t* state, cJSON* output)
{
    ...
    // unsigned long long* iat_arr = NULL;
    ...
    if (iat_count > 0 && iat_count <= max_len) {
        ...
        // iat_arr = (unsigned long long*)temp_buf;  // 仅用于熵和自相关计算
    }
    ...
    if (iat_count > 0 && iat_count <= max_len) {
        // 重新填充 iat_arr 用于计算
        unsigned long long* iat_data = (unsigned long long*)temp_buf;
        for (unsigned int i = 0; i < iat_count; i++) {
            unsigned long long* val = (unsigned long long*)circular_buffer_get(&state->iat_seq, i);
            if (val) iat_data[i] = *val;
        }

        cJSON_AddNumberToObject(output, "iat_entropy", calc_iat_entropy(iat_data, iat_count));
        cJSON_AddNumberToObject(output, "iat_autocorr_lag_1", calc_iat_autocorr(iat_data, iat_count, 1));
        cJSON_AddNumberToObject(output, "iat_autocorr_lag_2", calc_iat_autocorr(iat_data, iat_count, 2));
        cJSON_AddNumberToObject(output, "iat_autocorr_lag_3", calc_iat_autocorr(iat_data, iat_count, 3));
        cJSON_AddNumberToObject(output, "iat_autocorr_lag_4", calc_iat_autocorr(iat_data, iat_count, 4));
        cJSON_AddNumberToObject(output, "iat_autocorr_lag_5", calc_iat_autocorr(iat_data, iat_count, 5));
        ...
        if (iat_count > 1) {
            ...
            for (unsigned int i = 1; i < iat_count; i++) {
                if (iat_data[i - 1] > 0) {
                    ratio_sum += (double)iat_data[i] / iat_data[i - 1];
                    ratio_n++;
                }
            }
