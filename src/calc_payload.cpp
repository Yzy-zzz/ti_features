...
void calc_derived_payload(flow_feature_state_t* state, cJSON* output)
{
    ...
    unsigned int* payload_sizes = (unsigned int*)circular_buffer_get_array(&state->payload_size_seq, &payload_size_count);
    ...
    if (state->payload_total_bytes > 0) {
        ...
        cJSON_AddNumberToObject(output, "payload_non_empty_ratio",
            (double)state->payload_non_empty_count / state->payload_total_bytes);
    }
    ...
    // === 新增：Payload 字节值统计（256 个 bin 的完整直方图） ===
    // 注意：需要在 flow_state 中添加 payload_byte_hist_full[256]
    ...
    if (state->payload_size_diff_stats.count > 0) {
        ...
        cJSON_AddNumberToObject(output, "payload_size_diff_mean", 0);
        cJSON_AddNumberToObject(output, "payload_size_diff_std", 0);
        cJSON_AddNumberToObject(output, "payload_size_diff_min", 0);
        cJSON_AddNumberToObject(output, "payload_size_diff_max", 0);
        cJSON_AddNumberToObject(output, "payload_size_diff_skew", 0);
        cJSON_AddNumberToObject(output, "payload_size_diff_kurt", 0);
        cJSON_AddNumberToObject(output, "payload_size_diff_median", 0);
        cJSON_AddNumberToObject(output, "payload_size_diff_q1", 0);
        cJSON_AddNumberToObject(output, "payload_size_diff_q3", 0);
    }
