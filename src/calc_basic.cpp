...
void calc_derived_basic(flow_feature_state_t* state, cJSON* output)
{
    ...
    if (state->fwd_payload_count > 0 && state->fwd_payload_count <= max_len) {
        ...
        // 复制到临时 buffer 并排序
        for (unsigned int i = 0; i < count; i++) temp_buf[i] = (double)src[i];
        sort_small_uint_array((unsigned int*)temp_buf, count);
        ...
        cJSON_AddNumberToObject(output, "fwd_payload_median", calc_median_uint((unsigned int*)temp_buf, count));
        cJSON_AddNumberToObject(output, "fwd_payload_q1", calc_q1_uint((unsigned int*)temp_buf, count));
        cJSON_AddNumberToObject(output, "fwd_payload_q3", calc_q3_uint((unsigned int*)temp_buf, count));
    }
    ...
    if (state->bwd_payload_count > 0 && state->bwd_payload_count <= max_len) {
        ...
        cJSON_AddNumberToObject(output, "bwd_payload_median", calc_median_uint((unsigned int*)temp_buf, count));
        cJSON_AddNumberToObject(output, "bwd_payload_q1", calc_q1_uint((unsigned int*)temp_buf, count));
